...
const DOOR_STAMP = [
  'sh',
  '-c',
  '{ mkdir -p /tmp/code-runner && date -u +%Y-%m-%dT%H:%M:%SZ > /tmp/code-runner/door-activity; } 2>/dev/null; n=0; until tmux -S /tmp/code-runner/tmux.sock has-session -t agent 2>/dev/null; do n=$((n+1)); if [ \"$n\" -ge 90 ]; then echo \"code mode: terminal did not become ready within 90s; check the Host logs\" >&2; exit 1; fi; sleep 1; done; exec "$@"',
  'door',
];
...
/** A pod-shaped handle: the exec dialect only the driver knows . */
