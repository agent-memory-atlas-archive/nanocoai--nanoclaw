  ...
  try {
    ...
    await matrix.attempt('copy-to-operator-clipboard', async () => {
      ...
      return seen
        ...
        : ['FAIL', `no OSC 52 in the client stream (set-clipboard=${setting}) — copy stays in the pod's tmux buffer`];
    });
...
async function checkDisconnect(stack: TmuxStack): Promise<void> {
  ...
  matrix.row(
    ...
    'MANUAL',
    'a wedged-but-open exec stream blocks the in-pod tmux client on its tty writes; tmux buffers per-client server-side and the D14 idle lease stays the designed backstop — not reproducible under the auto-draining Bun.Terminal wrapper; verify on the kubectl leg',
  );
...
async function main(): Promise<void> {
  ...
  matrix.note(
    ...
    'A blocked client tty can leave a tmux client present; the D14 idle lease remains the designed backstop either way — verify on the kubectl leg.',
    ...
    'The kubectl leg (exec-stream latency, resize coalescing, POC bun/tmux versions) is out of local scope, and requires a separate live check.',
  );
  matrix.print();
