...
it('preserves operator credential settings without installing a helper', () => {
  fs.writeFileSync(settingsPath, JSON.stringify({ apiKeyHelper: '/operator/own-helper' }));
  ensureMailboxHooks(settingsPath);
  expect(read().apiKeyHelper).toBe('/operator/own-helper');
